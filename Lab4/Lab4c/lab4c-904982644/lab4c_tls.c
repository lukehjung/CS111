...
void printer(const char * string, int server)
{
	...
	{	
		...
		sprintf(buf, "%s\n", string);
		SSL_write(ssl, buf, strlen(string) + 1);
...
void connect_to_server()
{
	...
	SSL_library_init(); // loads encryption and hash algorithms
	SSL_load_error_strings(); // loads errors strings
	OpenSSL_add_all_algorithms(); // loads rest of algorithms

	SSL_CTX * context = SSL_CTX_new(TLSv1_client_method()); // create new SSL_CTX object
	if(context == NULL)
	{
	...
	ssl = SSL_new(context); // create new ssl struct to keep data

	if(ssl == NULL)
	{
		fprintf(stderr, "Error: Could not initalize SSL.\n");
		exit(2);
	}

	if (!SSL_set_fd(ssl, sock)) // set the ssl together with the sock fd
	{
		fprintf(stderr, "Error:  Could not set sock fd with SSL.\n");
		exit(2);
	}
	if (SSL_connect(ssl) != 1)  // connect ssl and make sure things run smoothly
	{
		fprintf(stderr, "Error: Could not connect SSL.\n");
		exit(2);
	}
}
